...
export default function AmbientBackground({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`pointer-events-none fixed inset-0 -z-10 overflow-hidden ${className}`} aria-hidden="true">
      {/* Faceted Crystal Obsidian Violet Base */}
      <div className="absolute inset-0 bg-[#120e18]" />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(60% 50% at 15% 20%, rgba(94,203,230,0.12), transparent 65%)," +
            "radial-gradient(55% 55% at 85% 25%, rgba(214,110,165,0.14), transparent 62%)," +
            "radial-gradient(70% 60% at 50% 100%, rgba(255,210,105,0.08), transparent 68%)",
        }}
      />
...
function runMainThreadFallback(canvas: HTMLCanvasElement) {
  ...
  const render = () => {
    if (!running) return;
    ctx.fillStyle = "rgba(18, 14, 24, 0.1)";
    ...
    for (const p of particles) {
      ...
      ctx.strokeStyle = "rgba(94,203,230,0.2)";
      ctx.beginPath();
      ctx.moveTo(p.x, p.y);
