"use client";
import React from "react";

interface RotatingHeroEmblemProps {
  className?: string;
}

export default function RotatingHeroEmblem({ className = "" }: RotatingHeroEmblemProps) {
  return (
    <div className={`relative flex items-center justify-center ${className}`} style={{ perspective: "600px" }}>
      {/* Outer ambient pulsing glow halo */}
      <div className="absolute w-8 h-8 rounded-full bg-gradient-to-r from-[#38bdf8]/40 via-[#62d3ee]/40 to-[#e580b5]/40 blur-md animate-pulse -z-10" />

      {/* 1/10th scale container with longitudinal Y-axis rotation and pulsing glow */}
      <div className="relative w-6 h-6 sm:w-7 sm:h-7 rounded-full p-0.5 border border-[#38bdf8] shadow-[0_0_12px_rgba(56,189,248,0.65)] bg-[#070d18] emblem-longitudinal-pulse flex items-center justify-center overflow-hidden aspect-square">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/assets/brain-lab-emblem-official.png"
          alt="Brain Lab by Liliya emblem"
          className="w-full h-full object-contain aspect-square rounded-full"
        />
      </div>
    </div>
  );
}
